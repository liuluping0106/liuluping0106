		...
		var txturl = getQueryVariable('txt');
		if(txturl){
			txturl = 'https://cdn.jsdelivr.net/gh/liuluping0106/liuluping0106/md5/doc/'+ txturl + ".txt";
			// alert(txturl);
			// 对应txt导入内容
			var xmlhttp;
			function loadXMLDoc(url,soyo)
			{
			...
			xmlhttp.onreadystatechange=soyo;
			xmlhttp.open("GET",url,true);
			xmlhttp.send();
			}

			loadXMLDoc(txturl,function()
			{
				if (xmlhttp.readyState==4 && xmlhttp.status==200)
				{
				strHTML="";
				var entext = xmlhttp.responseText;
				// alert(entext)
				strHTML+=entext;
				}
			});
